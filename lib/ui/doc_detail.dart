...
class DocDetailState extends State<DocDetail> {
  ...
  void _initCtrls() {
    ...
    fqYearCtrl = widget.doc.fqYear != null
        ? ut.Val.intToBool(widget.doc.fqYear!)
        : false;

    fqHalfYearCtrl = widget.doc.fqHalfYear != null
        ? ut.Val.intToBool(widget.doc.fqHalfYear!)
        : false;

    fqQuarterCtrl = widget.doc.fqQuarter != null
        ? ut.Val.intToBool(widget.doc.fqQuarter!)
        : false;

    fqMonthCtrl = widget.doc.fqMonth != null
        ? ut.Val.intToBool(widget.doc.fqMonth!)
        : false;
  }
  ...
  Future _chooseDate(BuildContext context, String initialDateString) async {
    ...
    var initialDate = ut.DateUtils.convertToDate(initialDateString) ?? now;
    ...
    DatePicker.showDatePicker(context, showTitleActions: true,
      ...
      setState(() {
        DateTime dt = date;
        String r = ut.DateUtils.ftDateAsStr(dt);
...
// Save doc
  void _saveDoc() {
    ...
    widget.doc.fqYear = ut.Val.boolToInt(fqYearCtrl);
    widget.doc.fqHalfYear = ut.Val.boolToInt(fqHalfYearCtrl);
    widget.doc.fqQuarter = ut.Val.boolToInt(fqQuarterCtrl);
    widget.doc.fqMonth = ut.Val.boolToInt(fqMonthCtrl);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        appBar: AppBar(
            title:
                Text((ttl != null || ttl!.isNotEmpty) ? ttl : "New Document"),
            actions: (ttl == "")
                ? <Widget>[]
                : <Widget>[
                    PopupMenuButton(
              ...
              child: ListView(
                ...
                children: <Widget>[
                  TextFormField(
                    ...
                    style: tStyle,
                    validator: (val) => ut.Val.validateTitle(val),
                  ...
                  Row(children: <Widget>[
                    Expanded(
                      ...
                      decoration: InputDecoration(
                          ...
                          hintText:
                              'Expiry date (i.e. ${ut.DateUtils.daysAheadAsStr(daysAhead)})',
                          labelText: 'Expiry Date'),
                      keyboardType: TextInputType.number,
                      validator: (val) => ut.DateUtils.isValidDate(val!)
                          ? null
